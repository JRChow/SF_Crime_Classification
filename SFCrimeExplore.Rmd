...
```{r}
library(ggplot2)
train_with_outlier <- train
train <- train[!(train$Y > 40),]
g <- ggplot(train, aes(X, Y))
g + geom_jitter(aes(colour = Category))

```
```{r}
train_with_outlier <- read.csv(file="train.csv", na.strings=c(""))
h <- ggplot(train_with_outlier, aes(X, Y))
h + geom_jitter(aes(colour = Category))
```
