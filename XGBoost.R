...
set.seed(1234)
train_index <- sample(1:nrow(train_one_hot), nrow(train_one_hot)*0.7)
train_label <- train_one_hot$Category[train_index]
#test_label <- train$Category[-train_index]
train_data <- train_one_hot[train_index, !(names(train_one_hot)=="Category")]
test_data  <- test_one_hot[, !(names(train)=="Category")]
train_matrix <- xgb.DMatrix(data = data.matrix(train_data), label = train_label)
test_matrix <- xgb.DMatrix(data = data.matrix(test_data))
#test_matrix <- xgb.DMatrix(data=data.matrix(test))

numberOfClasses <- length(unique(cats))
...
bst_model <- xgb.train(params = xgb_params,
                       data = train_matrix,
                       nrounds = nround)
save(bst_model, file="bst_model_one-hot.rda")

#load("bst_model_one-hot.rda")
test_pred <- predict(bst_model, newdata = test_matrix, type = "class")

#save(test_pred, file="bst_pred.rda")

test_prediction <- matrix(test_pred, nrow=nrow(test), ncol=numberOfClasses)
save(test_prediction, file="xgboost_pred.rda")
test_prediction <- as.data.frame(test_prediction)
id <- data.frame(Id=seq.int(from=0, to=nrow(test)-1))
names(test_prediction) <- sort(unique(cats))
submit <- cbind(id, test_prediction)
